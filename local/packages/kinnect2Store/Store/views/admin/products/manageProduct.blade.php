    ...
    <div class="product-Analytics">
        ...
        @endif
            ...
            <div class="selectdiv">
                ...
                <select class="sub-category selectList" id="sub_category" name="sub_category" required
                ...
                @if($url_user_id == Auth::user()->id)
            		<!--<a class="blueBtn fltR add-product" href="{{ url('store/'.$user->username.'/admin/add-product') }}">Add Product</a>-->
            	@endif
            </div>
            <div class="ProductCont">
                ...
                <div id="listing_products">
                    ...
                    <div class="notify p10">No product added yet. Please click on add product button to add the products.</div>
                    @else
                        @foreach($AllProducts as $Product)
                            <div class="productList  product_item_{{$Product->id}}">
                                ...
                                <div class="titleW">
                                    <a class="titleW"
                                       href="{{url('store/'.Auth::user()->username.'/product/'.$Product->id )}}">{{$Product->title}}</a>
                                </div>
                                <div class="priceW"><span>${{$Product->price}}</span></div>
                                {{--<div class="totalW">{{$Product->quantity}}</div>--}}
                                <div class="availW">{{$Product->quantity}}</div>
                                <?php $sold = $Product->sold; ?>
                                <div class="soldW">{{$sold}}</div>
                                <div class="actW" >
                                    <a class="stats" href="{{url('store/'.Auth::user()->username.'/admin/'.$Product->id )}}/product_analytics">Stats</a>
...
<div class="modal-box iwillchange" id="">
    <a href="#" class="js-modal-close close">�</a>
    <div class="modal-body">
        <div class="edit-photo-poup">
            <h3 style="color: #0080e8;">Delete Product</h3>
            <p class="mt10" style="width: 315px;height: 26px;line-height: normal">Are You Sure You Want To delete this product? </p>
...
<script type="text/javascript">
        ...
        jQuery.ajax({
            ...
            success: function (data) {
                ...
                $.each(maArray, function (key, val) {
                    ...
                    ProductsHtml += '<div class="availW">' + val.quantity + '</div>';
                    ProductsHtml += '<div class="soldW">' + val.sold + '</div>';
                    if (Auth_id == val.owner_id) {
                        ProductsHtml += '<div class="actW">';
                        ProductsHtml += '<a class="stats"  title="Stat" href="{{url('store/'.Auth::user()->username.'/admin/')}}/'+val.id+'/product_analytics">' + '</a>';
                ...
                if (ProductsHtml != '<div>') {
                    ...
                    $("#listing_products").html('<h1 class="productList" id="nop" >No Product found</h1>');
                }
            }, error: function (xhr, ajaxOptions, thrownError) {
                alert("ERROR:" + xhr.responseText + " - " + thrownError);
            }
    ...
    $(document).on("click", ".deleteProduct", function (e) {
        ...
        $(".iwillchange").attr("id", 'popup2-' + product_id);
        $(".iwillchange").show();
        $('#yes').click(function () {
            jQuery.ajax({
                type: "Post",
