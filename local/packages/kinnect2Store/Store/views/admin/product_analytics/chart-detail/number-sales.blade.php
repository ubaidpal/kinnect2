...
<div id="number-views" class="cssPopup_overlay">
    <div class="cssPopup_popup">
        ...
        {!! Form::open(['url' => "store/$user->username/admin/product-analytics/number-sales/$product_id", 'method'=>'post', 'enctype'=>"multipart/form-data", 'id' =>'number-form']) !!}


        <div class="bank-detail-popup" style="width: 100%">
            <div class="ana-detail-title">Total Number of Sales</div>
            <div class="bd-itm">
                <div class="bd-itml">Select Month:</div>
                <div class="bd-itmr">
                    <input type="text" placeholder="Select Month" name="month" required id="month">
                </div>
            </div>
        ...
        $("#getDayView").click(function(e){
            ...
            $.ajax({
                type : 'POST', url : $('#number-form').attr('action'), data : data, success : function(resultD){
                    console.log(dayView.options.data[0].dataPoints)
                    //var points = [];
