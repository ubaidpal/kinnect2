                ...
                @foreach($albums as $album)
                    ...
                    @if($user->id === Auth::user()->id || $privacy)
                        <li>
                            ...
                            <a class="txt fltL" href="{{url("albums/photos/".$album->album_id)}}"
                               title="Profile Photo">{{$album->title}}</a>
                            @if($album->type != 'profile' && $album->owner_id == Auth::user()->id)
                                <a class="fltR mt5 delete-album" href="{{url('albums/delete/'.$album->album_id)}}">Delete</a>
                            @endif
                            <div class="clrfix"></div>
