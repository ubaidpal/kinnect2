...
<div class="full-width-banner">
            ...
            @if($user->user_type == 2)
                @if($user->id == Auth::user()->id)
                    @if(env("STORE_ENABLED", TRUE) && isStoreHaveProducts( $user->username ) == 1)
                        @if(Auth::user()->brand_detail->store_created == 1 )
                            <!--<a href="{{url('/store/'.$user->username)}}"
                               title="Preview your store of {{$user->displayname}}">Store Preview</a>-->
                        @endif
                    @endif

                    @if(env("STORE_ENABLED", TRUE))
                        @if(Auth::user()->brand_detail->store_created == 0)
                            <a data-modal-id="agree-condition" href="#"
                               title="{{$user->displayname}} go to Add category and enable your store">
                                Create Store
                            </a>
                        @else
                            <a href="{{url('store/'.$user->username.'/admin/categories/')}}"
                               title="{{$user->displayname}} go to Admin area of your store">Admin</a>
                        @endif
                    @endif
                @else
                    @if(env("STORE_ENABLED", TRUE) && isStoreHaveProducts( $user->username ) == 1)
                        <a href="{{url('/store/'.$user->username)}}"
                           title="Go to store of {{$user->displayname}}">Store</a>
                    @endif
                @endif
            @endif
        </div>
        <div class="banner-links @if($user->user_type == 2) store @endif" data-user="{{$user->username}}" data-url="/profile/profile-view"
            ...
            {{-- @if($user->user_type == Config::get('constants.REGULAR_USER'))--}}
            <a href="javascript:void(0);" title="Following" class="tab" data-target="following" data-ajax="true">
                Following({{@$following}})
            </a>
            {{-- <@endif--}}
        ...
        <div class="p_btn_div">
            ...
            @elseif(Auth::user()->id != $user->id &&  is_friend_request_received(Auth::user()->id, $user->id))
                ...
                <a class="friend-toggle @if(!allowed_to_add_kinnector($user->id,Auth::user()->id,$user->user_type,Auth::user()->user_type)) noToggleBtn @endif"
                   href="{{URL::to('friends/delete/'  .$user->id)}}">
                    <span class="delete-request"> Delete Request</span>
                </a>
            ...
            @elseif(Auth::user()->id != $user->id &&  !is_friend(Auth::user()->id, $user->id) > 0 AND ((Auth::user()->user_type == $regular AND $user->user_type != $brand) OR ((Auth::user()->user_type == $brand AND $user->user_type == $regular))) )
                ...
                <a class="friend-toggle" href="{{URL::to('friends/add-friend/'  .$user->id)}}">
                    <span class="add_frnd"> Add Friend</span>
                </a>
                ...
                <a href="javascript:void(0);" title="Invite users to follow this brand" class="js-open-modal"
                   data-modal-id="popup2">
                    @if(Auth::user()->user_type == Config::get('constants.BRAND_USER'))
                        <span class="add_frnd">Invite People</span>
                    @else
                        <span class="add_frnd">Invite Friends</span>
                    @endif
                </a>
    ...
    <div class="cover-photo-container" id="cover_photo_div">{{$user->cover_photo_id}}
