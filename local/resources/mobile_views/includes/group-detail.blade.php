    ...
    <div class="sub-nav-container">
        ...
        @if((Kinnect2::isFollowingGroup($group['id'],Auth::user()->id) > 0) OR ($perms['permission'] == Config::get('constants.PERM_EVERYONE')))
            @if($perms['permission'] != Config::get('constants.PERM_GROUP_OFFICERS_AND_OWNERS'))
                <a href="javascript:void(0);" title="What's New" class="sub-nav-item tab @if(Request::is('*/group/*')) active @endif" data-target="whats-new" data-ajax="true"><div  class="sub-nav-txt tab">What's New</div></a>
            @else
        ...
        <a href="#" class="@if(Request::is('*/group/members') || Request::is('group/members/*')) active @endif sub-nav-item tab" title="See all group members" data-target="members" data-ajax="true">
            <div  class="sub-nav-txt tab">Members({{ Kinnect2::followers($group['id']) }})</div>
        </a>
        ...
        @if((Kinnect2::isFollowingGroup($group['id'],Auth::user()->id) > 0) OR ($perm['permission'] == Config::get('constants.PERM_EVERYONE')))
            <a href="{{url('group/events/')}}/{{$group['id']}}" class="@if(Request::is('*/group/events') || Request::is('group/events/*')) active @endif sub-nav-item tab" title="See all events about this group." data-target="events" data-ajax="true">
                <div  class="sub-nav-txt tab">Events ({{ Kinnect2::countEvents('group', $group['id']) }})</div>
            </a>
        @endif
        <a href="{{url('group/info/')}}/{{$group['id']}}" class="sub-nav-item tab @if(Request::is('*/group/info') || Request::is('group/info/*')) active @endif" title="See all information about this group." data-target="info" data-ajax="true">
            <div  class="sub-nav-txt tab">Info</div>
        </a>
    </div>-->
    <style>
        #croppic{
