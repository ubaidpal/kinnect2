    ...
    <div class="content-gray-title mb10" data-user="{{$group['id']}}"  data-url="/group/profile-content">
        ...
        @if((Auth::user()->id == $group->creator_id) ||($isAuthGroupManager == 1))
            <a data-target="pending-invites" data-ajax="true" title="See Pending Invites" class="btn fltR tab">Pending Invites</a>
            @if($group->approval_required == 1)
                <a data-target="pending-requests" data-ajax="true" title="See Pending Requests" class="btn fltR tab" style="margin-right: 5px;">Pending Requests</a>
            @endif
        @endif
    </div>
        ...
        @foreach($members as $member)
            <div class="browse-battle-item">
                <div class="btn-del">
                  ...
                  <img src="{{ Kinnect2::getPhotoUrl($groupOwner->photo_id, $groupOwner->id, 'user', 'thumb_normal')}}" alt="image">
                </a>
                </div>
                        ...
                        <div class="item-vote">
                            ...
                            @if($isGroupManager == 0)
                                ...
                                @if(Kinnect2::IsGroupManagerReq($group['id'],$member->user_id) > 0)
                                    <a class="btn btn-add-friend fltR" onclick="demote_manager('<?php echo $group->id ?>','<?php echo $member->user_id ?>')"
                                    id="{{$member->user_id}}">Cancel Promotion</a>
                                @else
                                    <a class="btn btn-add-friend fltR" onclick="make_manager('<?php echo $group->id ?>','<?php echo $member->user_id ?>')"
                                    id="{{$member->user_id}}">Make Manager</a>
                                @endif
                            @else
                                @if(Auth::user()->id == $member->user_id)
                                    ...
                                    <a class="btn btn-add-friend fltR" onclick="demote_manager('<?php echo $group->id ?>','<?php echo $member->user_id ?>')"
                                    id="{{$member->user_id}}">Self Demote</a>
                                @else
                                    <a class="follow-btn btn btn-add-friend fltL mr10"
                                    onclick="remove_member('<?php echo $group->id ?>','<?php echo $member->user_id ?>', event, this)"
                                    id="{{$member->user_id}}">Remove</a>
                                    <a class="btn btn-add-friend fltR" onclick="demote_manager('<?php echo $group->id ?>','<?php echo $member->user_id ?>')"
                                    id="{{$member->user_id}}">Demote</a>
                                @endif
                            @endif
                        </div>
  ...
  <script>
    function make_manager(group_id, member_id) {
        var dataString = {
            group_id: group_id,
            member_id: member_id
        };
    ...
    function demote_manager(group_id, member_id) {
        var dataString = {
            group_id: group_id,
            member_id: member_id
        };
