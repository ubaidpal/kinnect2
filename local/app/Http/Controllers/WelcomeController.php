...
class WelcomeController extends Controller
{
    ...
    */
     ...
     * @return void
     */
    protected $notTruncate = ['album_categories', 'brand_industries', 'countries', 'event_categories', 'group_categories', 'oauth_clients', 'time_zone'];
    ...
    public function index()
    {
        ...
        foreach ($tableNames as $table) {
            if (!in_array($table, $this->getNotTruncate())) {
                \DB::statement('SET FOREIGN_KEY_CHECKS=0;');
                \DB::table($table)->truncate();
            }
        }
        \DB::statement('SET FOREIGN_KEY_CHECKS=1;');
        dd('Sir Zahid! Everything gone now');
    }

    /**
     * @return mixed
     */
    public function getNotTruncate()
    {
        return $this->notTruncate;
    }
    ...
    {
        echo Carbon::now('Asia/Karachi')->diffInHours(Carbon::now('Europe/London'));
        $timezone = $this->getTimezoneList();
        foreach($timezone as $key => $row){
            \DB::table('time_zone')->insert(['country' => $key, 'value'=>$row]);
        }
        dd('Sir Zahid!All Done');
    }
