    ...
    function initiateChat(io, util){
                            ...
                            if(this.sender_id == loggedInSocket){
                                if(this.file_id){
                                    ...
                                    var attachmentOptions = {
                                        ...
                                        from : loggedInSocket,
                                        mime_type : this.file_meta.mime_type,
                                        to : uid,
                                        self : true,
                                        oldMessage : true,
                                        isGroup : false,
        ...
        function sendAttachment(options){
            ...
            if(formData){
                ...
                $.ajax({
                    url : ajaxPathPrefix + "messages/store", // uploadAttachment
        ...
        function attachmentHtml(options){
            ...
            if(typeof mime_type != 'undefined' && mime_type.indexOf('image',0) != -1) {
                options['content'] = '<a  download="" href="' + filePath + '"><img src="' + filePath + '" width="80"/></a>';
                options['content'] += "<a class='attachment-file' download='' href='" + filePath + "'>" + fileName + "</a>";
            }else if(typeof mime_type != 'undefined' && mime_type.indexOf('audio',0) != -1){
                options['content'] = "<a class='attachment-file' download='' href='" + filePath + "'>" + fileName + "</a>";
                options['content'] += '<div class="clrfix"></div><span class="mt5"><audio style="width: 205px;" src="' + filePath + '" controls></audio></span>';
            }else{
                options['content'] = "<a class='attachment-file' download='' href='" + filePath + "'>" + fileName + "</a>";
            }
            if(options.isGroup){
        ...
        function addFriendOnline(data){
            setTimeout(function(){
                ...
                $('#friends_online').append('<li style="'+displayStyle+'" class="' + userClass + ' allOnlineFriends" data-socket="' + data.user_id + '"><img src="' + user.profile_pic + '" width="34px" title="' + data.displayname + '">' + data.displayname + '</li>');
        ...
        $(document).ready(function(){
            ...
            $(".self-online-status").click(function(e){
               // $(".friends-list-wrapper").toggle();
                ...
                if(!currentStatus){
                    ...
                    $("#chat_windows .chat-window").each(function(){ $(this).remove()})
                }else{
                    dEl.addClass("online");
            ...
            $(document).on('click', function(e){
                var clsNotClopsAble = ['chat-window-user', 'chat-settings', 'add-to-chat', 'chat-window-display-name', 'online-friends-header', 'chatFriendsToShow active', 'kinnector allOnlineFriends', 'brand allOnlineFriends', 'conv-chat-trigger', 'online-chat-friend', 'online-friends-count', 'btn fltR mr5 chat-trigger message-btn', 'close-chat','settings-panel','settings-options','leave-it', 'edit-name','name-update','group-name-field','update-name-btn'];
