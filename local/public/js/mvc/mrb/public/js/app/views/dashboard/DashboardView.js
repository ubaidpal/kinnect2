define( ['App', 'backbone', 'marionette', 'jquery', 'underscore' , 'models/Model', 'text!templates/dashboard/main.html', 'views/dashboard/ChildView', 'views/dashboard/fileUploads/ContainerView', 'views/dashboard/linksPreview/linksPreview'],
    function(App, Backbone, Marionette, $, _, Model, template, ChildView, UploadsContainerView, LinksPreview) {
        ...
        return Backbone.Marionette.CompositeView.extend( {
            ...
            itemView: ChildView,
            itemViewContainer: ".posts-container",
            ...
            serializeData: function(){
                ...
                var postTargetId = k2App.collections.posts.postTargetId;
                var profileUserId = $("#profileLink").data("username");
                var postTarget = k2App.collections.posts.postTarget;
                var upload_permisson = $("#group-details-wrapper").data("upload-permission");
                data.write_post = true;

                if(typeof postTargetId != 'undefined' && typeof profileUserId != 'undefined' && postTargetId != profileUserId && postTarget != 'group'){
                    data.write_post = false;
                }else if (postTarget == 'group' && !upload_permisson){
                    data.write_post = false;
                }
                return data;
            },
            ...
            events: {
                ...
                "click .edit-post-save" : "editSave",
                "click .close-edit-modal" : "closeEditPopup",
                "click .post-likes-tab" : "getPostLikes",
                "click .post-dislikes-tab" : "getPostDislikes",
            },
            getPostLikes : function (e) {
                e.preventDefault();
                jQuery('#likes_container').fadeOut(500);
                var post_id = $(e.target).data('id');
                k2App.collections.posts.getPostLikes({
                    postID : post_id,
                    successCallback : function(response){
                        jQuery('#likes_container').fadeIn(500);
                        myHtml = '<div class="like-box">';
                        myHtml += '<div class="like-box-heading">';
                        myHtml += '<span CLASS="active">Likes</span>';
                        myHtml += '<a href="#" class="post-dislikes-tab">Dislikes</a>';
                        myHtml += '</div>';
                        jQuery.each(response.likes,function (elem,obj) {
                            myHtml += '<div>';
                            myHtml += '<a href="'+response.base_url+'/profile/'+obj.liker.username+'">';
            ...
            getPostLikes : function (e) {
                ...
                k2App.collections.posts.getPostLikes({
                    ...
                    successCallback : function(response){
                        ...
                        myHtml += '<div class="like-box-heading">';
                        myHtml += '<span class="active">Likes</span>';
                        myHtml += '<a href="#" data-id="'+post_id+'" class="post-dislikes-tab" >Dislikes</a>';
                        myHtml += '</div>';
            ...
            getPostDislikes : function (e) {
                ...
                jQuery('#likes_container').fadeOut(500);
                var post_id = $(e.target).data('id');
                k2App.collections.posts.getPostDislikes({
                    postID : post_id,
                    successCallback : function(response){
            ...
            editSave : function () {
                var post_text = jQuery('#post_edit_text').val();
                post_text = jQuery.trim(post_text);
                var dataObj = {};
                // dataObj.tokens = [];
                // dataObj.tokens_old = [];
                // var context = this;
                //
                // if($('.edit-post-save').data('executing'))
                // {
                //     return false;
                // }
                // $.each(k2App.collections.fileUploads.toArray(), function(){
                //     if(this.get('file_type') == 'old'){
                //         dataObj.tokens_old.push(this.get('token'));
                //     }else if(this.get("token")){
                //         //dataString += "&token[]="+ this.get("token");
                //         dataObj.tokens.push(this.get("token"));
                //     }else if(this.get("type") == 'video'){
                //         $('.edit-post-save').data('executing',true);
                ...
                // }
                //if(dataObj.tokens_old.length > 0 || dataObj.tokens.length > 0 || post_text || typeof dataObj.link == 'object'){
                if(post_text){
                    $('.edit-post-save').data('executing',true);
                    //$(".disable-required").attr("disabled", true).css("color", "lightgrey");
                    dataObj.post_body = post_text;
                    dataObj.action_id = jQuery('#editPostIdentifier').val(),
                    k2App.collections.posts.editStatus({
                        data: dataObj,
                        view : this,
                        successCallback: function(res){
                            ...
                            $(".modal-box, .modal-overlay").fadeOut(500, function () {
                                ...
                                k2App.collections.posts.where({post_id:res.post.post_id })[0].set(res.post);
                            });
                        }
                    });
                }
            ...
            flagActivity : function (e) {
                e.preventDefault();
                jQuery('#flag_error').hide();
                post_id = jQuery('#postIDentifier').val();
                category = jQuery('#category').val();
                report_text = jQuery('#report_text').val();
                report_text = jQuery.trim(report_text);
                
                if(!report_text){
                    jQuery('#flag_error').show();
                    return false;
                }
                $(".modal-box, .modal-overlay").fadeOut(500, function () {
            ...
            savePostSuccessCallback: function(res){
                ...
                $("#k2post").val("");
                if(typeof res.message != 'undefined' && (res.message == 'video_uploaded' || res.message == 'audio_uploaded') ){
                        ...
                        $.ajax({
                            ...
                            .done(function(res) {
                                if(res.error && res.error == 1){
                                    ...
                                    jQuery('#status-loader').hide();

                                    console.log(jQuery('#file_uploaded_msg').show().fadeOut(10000));
                                }
            ...
            getNextPage: function(){
                ...
                if(!App.postFetching){
                    App.postFetching = true;
                        ...
                        {
                            $('#page_end_message').html('No more records to show').css('display','');
                            return false;
