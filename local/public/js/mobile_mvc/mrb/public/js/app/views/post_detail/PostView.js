    ...
    function(App, Backbone, Marionette, $, _, Model, Collection, template, ChildView) {
        ...
        return Backbone.Marionette.CompositeView.extend( {
            ...
            events: {
                ...
                "click .post-close-cta": "backToDashboard",
            ...
            VoteBattle : function(e)
            {
                ...
                if(option)
                {
                    k2App.collections.posts.voteBattle({
                        option : option,
            ...
            votePoll : function(e){
                ...
                if(option)
                {
                    k2App.collections.posts.votePoll({
                        option : option,
                        postID : this.model.get('post_id'),
                        view: this,
                        successCallback : function(response,model, view){
                ...
                k2App.collections.posts.dislikePost(
                    {
                        ...
                        successCallback: function (collection, postID,response) {
                            $('.feed-item .like-post').removeClass('active');
                            $('.feed-item .dislike-post').addClass('active');
                            $('.feed-item .likes-count').text(response.likes.like_count + ' likes');
                            $('.feed-item .dislikes-count').text(response.likes.dislike_count+ ' dislikes');
                        }
                    }
                );
                ...
                k2App.collections.posts.undoDislikePost(
                    {
                        ...
                        successCallback: function (collection, postID,response) {
                            $('.feed-item .like-post').removeClass('active');
                            $('.feed-item .dislike-post').removeClass('active');
                            $('.feed-item .likes-count').text(response.likes.like_count + ' likes');
                            $('.feed-item .dislikes-count').text(response.likes.dislike_count+ ' dislikes');
                        }
                    }
                ...
                k2App.collections.posts.unlikePost(
                    {
                        ...
                        successCallback: function (collection, postID,response) {
                            ...
                            k2App.views.postDetail.model.set("post_disliked", 1);
                        }
                    }
                );
